...
function initData(){
    ...
    $('.dropdown').dropdown({
        "onChange": function(value){
            $("#ejemplo1,#ejemplo2,#ejemplo3,#ejemplo5").hide();
            if(value == "cecps"){
                $("#ejemplo1").show();
...
function makePrediction() {
    clearChart();
    var method = $('#predictionMethod').val(); 

    $.post('/prediccion/crear', {predictionMethod:method}, function(data) {
        var weeks = data['weeks'];
        var y = data['data'];
        var y_future = data['y_future'];
        var y_prediction = data['data_p'];
        var r2 = data['r^2'];
        var mse = data['mse'];
        var rmse = data['rmse'];

        y_prediction[0] = Number.NaN;
        y[y.length - 1] = Number.NaN;
        y_future = initFuture(y_future, y.length);
;
        ...
        myChart = new Chart("chart-pred", {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    label: 'Prediccion (3 semanas despues)', 
                    borderColor: "#000000",
                    borderWidth: 1.5,
                    fill: false,
                    data: y_future,
...
function initFuture(data, index) {
    y_future = []
    for (let i=0; i < index; i++) {
        y_future.push(Number.NaN);
    }

    y_future.push(data[0])
    y_future.push(data[1])
    y_future.push(data[2])
